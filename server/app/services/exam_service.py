...
class ExamService:
    ...
    def _update_topic_progress(self, topic: str, understanding_score: float, covered_points: List[str]):
        """更新主题进度"""
    ...
    def _calculate_response_consistency(self) -> float:
        ...
        session = self.state_machine.context["exam_session"]
        evaluations = [
            eval["metrics"]["understanding"] 
            for eval in session.evaluations.values()
        ]
        
        # 计算相邻评分的差异
        differences = [
            abs(evaluations[i] - evaluations[i-1]) 
